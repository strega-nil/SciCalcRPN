/*
	...
	Written in 2013 by Mazzuca, Nicole <nicole@strega-nil.co>
...
#include <stdio.h>
#include <stdlib.h>
#include <math.h> //I need this for pow()
#include "primes.h"
#include "frac.h"

/** Finds the length of a decimal number.
 *  
 *  This takes an integer (n), and returns how many chars
 *  a string holding this integer would need, plus a sign.
 *  (i.e. n = +10 would return 3).
 */
int ilen(int n)
{
	int i;
...
char* f2s(frac f)
{
	...
	if(f.den == 1){
		...
		str = malloc(sizeof(char) * (ilen(f.num) + ilen(f.den) + 1));
...
frac fsimp(frac f)
{
	...
	for(int i = 0; PRIMES[i] <= lpcf && i < PRIMESLENGTH; i++){
		while((f.num % PRIMES[i] == 0) && (f.den % PRIMES[i] == 0)){
			f.num = f.num / PRIMES[i];
			f.den = f.den / PRIMES[i];
			lpcf  = lpcf  / PRIMES[i];
		}
	}
 ...
 */
frac fpow(frac f1, int exp)
{
	f1.num = pow(f1.num, exp);
	f1.den = pow(f1.den, exp);
	return f1;
}
